...
export class MappingHelperService {
  ...
  static helperAddFolder(mapping?: StubMapping): StubMapping | undefined {
    if (mapping) {
      ...
      if (
        UtilService.isUndefined(mapping.metadata[UtilService.WIREMOCK_GUI_KEY])
      ) {
        mapping.metadata[UtilService.WIREMOCK_GUI_KEY] = {};
      }
      if (
        UtilService.isUndefined(
          mapping.metadata[UtilService.WIREMOCK_GUI_KEY][UtilService.DIR_KEY]
        )
      ) {
        mapping.metadata[UtilService.WIREMOCK_GUI_KEY][UtilService.DIR_KEY] =
          '/some/path';
  ...
  static helperAddDelay(mapping?: StubMapping): StubMapping | undefined {
    if (
      mapping &&
      mapping.response &&
      !mapping.response.fixedDelayMilliseconds
    ) {
  ...
  static helperAddHeaderRequest(
    mapping?: StubMapping
  ): StubMapping | undefined {
  ...
  static helperAddHeaderResponse(
    mapping?: StubMapping
  ): StubMapping | undefined {
  ...
  static helperAddRemoveProxyPathPrefix(
    mapping?: StubMapping
  ): StubMapping | undefined {
    if (
      mapping &&
      mapping.response &&
      !mapping.response.proxyUrlPrefixToRemove
    ) {
  ...
  static helperAddAdditionalProxyRequestHeaders(
    mapping?: StubMapping
  ): StubMapping | undefined {
    if (
      mapping &&
  ...
  static helperAddResponseTemplatingTransformer(
    mapping?: StubMapping
  ): StubMapping | undefined {
